...
const TechBlogMain = ({ icon, data, Heading, Para }) => {
  return (
    <div
      ...
      className="min-h-[40vh] flex flex-col md:flex-row items-center md:items-start justify-between"
    >
      {/* Left Section */}
      <div
        style={{ borderColor: darkTheme.colors.border }}
        className="flex flex-col items-center md:items-start md:w-[40%] md:border-b  xl:px-16 xl:py-[5.7rem] lg:px-10 lg:py-[2.95rem] md:px-4 md:py-[1.82rem] p-4"
      >
        {/* Logo */}
        <div className="mb-4">
          <img src={icon} alt="" />
        ...
        <h1
          ...
          className="text-4xl font-semibold mb-2 md:text-left text-center"
        >
          {Heading}
        </h1>
      ...
      <div
        style={{ borderColor: darkTheme.colors.border }}
        className="grid grid-cols-1 sm:grid-cols-2 gap-6  md:w-[60%] md:border-l xl:p-16 lg:p-10 md:p-4 md:border-b p-4"
      >
        {data.map((item, index) => (
          <div
            style={{
